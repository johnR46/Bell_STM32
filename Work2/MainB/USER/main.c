...
// get name bt value 
void viewName(void);
int nameArray[] = {0x41,0x54,0x2B,0x4E,0x41,0x4D,0x45,0x3F};  // AT+NAME?
int send___ = 1;
int main(void)
{
...
//---------------------------GetBTName bluetooth module--------------------
//
// now 
void viewName()
{
  int x = 0;
  while (x < sizeof(nameArray) / sizeof(int))
  {
		printf("%c",nameArray[x]);
    USART_SendData(UART4, nameArray[x++]);
    while (USART_GetFlagStatus(UART4, USART_FLAG_TC) == RESET)
    {
    }
  }
